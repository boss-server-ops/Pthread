...
void* threadFunc2(void* param)
{
    ...
    for (int i = 0; i < beixiaoyuannum; i++)
    {
        ...
        for (int j = 0; j < 5; j++)
        {
            ...
            {
                va = _mm_loadu_si128((__m128i const*)&beixiaoyuanmatrix[i][k]);
                vt = _mm_loadu_si128((__m128i const*)&xiaoyuanzimatrix[j][k]);
                va = _mm_xor_si128(va, vt);
                _mm_storeu_si128((__m128i*)&beixiaoyuanmatrix[i][k], va);
            }
            sem_post(&sem_main);
